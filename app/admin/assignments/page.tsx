            ...
            <div key={asgn.id} className="p-6 rounded-3xl glass-panel border space-y-4">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-slate-800 pb-3">
                <div>
                  ...
                  <span className="text-xs text-slate-400">المادة: {asgn.subject} | الدرجة الكلية: {asgn.maxScore}</span>
                </div>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-md">
            ...
            >
              <div className="flex items-center justify-between border-b border-slate-800 pb-3">
                <h3 className="text-base font-extrabold text-white">{dict.assignments.gradeSubmission}</h3>
              ...
              <form onSubmit={handleGradeSubmit} className="space-y-4 text-xs font-semibold">
                ...
                <div className="space-y-1">
                  <label className="text-slate-300">{dict.assignments.teacherFeedback}</label>
                  <textarea
                    required
                    rows={3}
                    value={teacherFeedback}
                    onChange={(e) => setTeacherFeedback(e.target.value)}
                    placeholder={dict.assignments.feedbackPlaceholder}
      ...
      <AnimatePresence>
        {isCreateModalOpen && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-md">
            ...
            >
              <div className="flex items-center justify-between border-b border-slate-800 pb-3">
                <h3 className="text-base font-extrabold text-white">{dict.assignments.createAssignment}</h3>
              ...
              <form onSubmit={handleCreate} className="space-y-4 text-xs font-semibold">
                <div className="space-y-1">
                  <label className="text-slate-300">عنوان التكليف</label>
                  <input
                    type="text"
                    required
                    value={title}
                    onChange={(e) => setTitle(e.target.value)}
                    placeholder="مثال: واجب تطبيقات التفاضل"
                ...
                <div className="space-y-1">
                  <label className="text-slate-300">الشرح والتفاصيل</label>
                  <textarea
                    required
                    rows={3}
                    value={description}
                    onChange={(e) => setDescription(e.target.value)}
                    placeholder="اكتب التوجيهات والمسائل المطلوب حلها..."
                    className="w-full p-3 rounded-xl bg-slate-950 border border-slate-800 text-white focus:outline-none focus:border-amber-500"
                  />
                </div>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div className="space-y-1">
                    <label className="text-slate-300">{dict.assignments.deadline}</label>
                    <input
                      type="datetime-local"
                      value={deadline}
                  ...
                  <div className="space-y-1">
                    <label className="text-slate-300">{dict.assignments.maxScore}</label>
                    <input
                      type="number"
                      value={maxScore}
